...
if nb_Households <> 1 & [Country == "France"]
	...
	if [H_DISAGG == "H20"]
		...
		Indice_Rich 	= 20;
		Name_Poor = "Poor (F0-5)";
		Name_Lower = "Lower class (F5-35)";
		Name_Middle = "Middle class (F35-65)";
		Name_Upper = "Upper class (F65-95)";
		Name_Rich = "Rich (F95-100)";
	end
	if [H_DISAGG == "H10"]
		Indice_Poor 	= 1;
		Indice_Lower	= 2:3;
		...
		Indice_Upper	= 8:9;
		Indice_Rich 	= 10;
		Name_Poor = "Poor (F0-10)";
		Name_Lower = "Lower class (F10-30)";
		Name_Middle = "Middle class (F40-70)";
		Name_Upper = "Upper class (F70-90)";
		Name_Rich = "Rich (F90-100)";
	end
	if [H_DISAGG == "L5"]
		Indice_Poor 	= 1;
		Indice_Lower	= 2;
		...
		Indice_Rich 	= 5;
		Name_Poor = "Rural";
		Name_Lower = "Small towns";
		Name_Middle = "Medium towns";
		Name_Upper = "Large towns";
	...
	OutputTable.EquityEfficiency = [..
		...
		[Name_Poor,									string(sum(BY.C_value(:, Indice_Poor))*money_disp_adj)+money_DUnit_short+money,		round(1000*(QInd_Fish( BY.pC, BY.C, Out.pC, Out.C, :, Indice_Poor)-1))/10 + " %"];..
		[Name_Lower,								string(sum(BY.C_value(:, Indice_Lower))*money_disp_adj)+money_DUnit_short+money,	round(1000*(QInd_Fish( BY.pC, BY.C, Out.pC, Out.C, :, Indice_Lower)-1))/10 + " %"];..
		[Name_Middle,								string(sum(BY.C_value(:, Indice_Middle))*money_disp_adj)+money_DUnit_short+money,	round(1000*(QInd_Fish( BY.pC, BY.C, Out.pC, Out.C, :, Indice_Middle)-1))/10 + " %"];..
		[Name_Upper,								string(sum(BY.C_value(:, Indice_Upper))*money_disp_adj)+money_DUnit_short+money,	round(1000*(QInd_Fish( BY.pC, BY.C, Out.pC, Out.C, :, Indice_Upper)-1))/10 + " %"];..
		[Name_Rich,									string(sum(BY.C_value(:, Indice_Rich))*money_disp_adj)+money_DUnit_short+money,		round(1000*(QInd_Fish( BY.pC, BY.C, Out.pC, Out.C, :, Indice_Rich)-1))/10 + " %"];..
		...
		[Name_Poor,									BY.H_disposable_income(Indice_Poor)/sum(BY.H_disposable_income),					100*(Out.H_disposable_income(Indice_Poor)/sum(Out.H_disposable_income)-BY.H_disposable_income(Indice_Poor)/sum(BY.H_disposable_income))];..
		[Name_Lower,								sum(BY.H_disposable_income(Indice_Lower))/sum(BY.H_disposable_income),				100*(sum(Out.H_disposable_income(Indice_Lower))/sum(Out.H_disposable_income)-sum(BY.H_disposable_income(Indice_Lower))/sum(BY.H_disposable_income))];..
		[Name_Middle,								sum(BY.H_disposable_income(Indice_Middle))/sum(BY.H_disposable_income),				100*(sum(Out.H_disposable_income(Indice_Middle))/sum(Out.H_disposable_income)-sum(BY.H_disposable_income(Indice_Middle))/sum(BY.H_disposable_income))];..
		[Name_Upper,								sum(BY.H_disposable_income(Indice_Upper))/sum(BY.H_disposable_income),				100*(sum(Out.H_disposable_income(Indice_Upper))/sum(Out.H_disposable_income)-sum(BY.H_disposable_income(Indice_Upper))/sum(BY.H_disposable_income))];..
		[Name_Rich,									BY.H_disposable_income(Indice_Rich)/sum(BY.H_disposable_income),					100*(Out.H_disposable_income(Indice_Rich)/sum(Out.H_disposable_income)-BY.H_disposable_income(Indice_Rich)/sum(BY.H_disposable_income))];..
