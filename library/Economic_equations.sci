...
function y = H_demand_Const_4(Consumption_budget, alpha_share_budget, sigma_demand, efficiency_coeff, C, pC, alpha_sigma) ;
    signRuben = sign(pC);
    pC = abs ( pC);
	Consumption_budget = abs(Consumption_budget);
    ...
    for i = 1:nb_Sectors
        alpha_sigma = alpha_share_budget(i).^ sigma_demand;
    end
