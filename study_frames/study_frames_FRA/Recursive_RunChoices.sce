...
case "AME"
	select time_step 
	case 1 then 
		parameters.phi_IC(Indice_NonEnerSect,Index_Gaz) = -0.1550108 * ones(parameters.phi_IC(Indice_NonEnerSect,Index_Gaz));
		parameters.phi_K(Index_Gaz)  = -0.1550108;
		parameters.phi_L(Index_Gaz)  = -0.1436035;
		parameters.phi_IC(Index_Agri,Index_Gaz) = 0.0; 
	case 2 then 
		parameters.phi_IC(Indice_NonEnerSect,Index_Gaz) = -0.0796872 * ones(parameters.phi_IC(Indice_NonEnerSect,Index_Gaz));
		parameters.phi_K(Index_Gaz)  = -0.0796872;
		parameters.phi_L(Index_Gaz)  = -0.0649622;
		parameters.phi_IC(Index_Agri,Index_Gaz) = 0.0;		 
	end
case "AMS"
	select time_step 
	case 1 then
		parameters.phi_IC(Indice_NonEnerSect,Index_Gaz) = 0.1161498 * ones(parameters.phi_IC(Indice_NonEnerSect,Index_Gaz));
		parameters.phi_K(Index_Gaz)  = 0.0662911;
		parameters.phi_L(Index_Gaz)  = 0.10112;	
		parameters.phi_IC(Index_Agri,Index_Gaz) = -0.1860797;
		Deriv_Exogenous.markup_rate = markup_rate;
		Deriv_Exogenous.markup_rate(Index_Gaz) = mean(markup_rate(Index_Carb));
		Deriv_Exogenous.Production_Tax_rate = Production_Tax_rate;
		Deriv_Exogenous.Production_Tax_rate(Index_Gaz) = mean(Production_Tax_rate(Index_Carb));
